resource "circonus_stream_group" "catalog-service-queries" {
  ...
  group {
    query = "consul`consul`catalog`service`query`*"
    type = "average"
  }
...
resource "circonus_stream_group" "catalog-service-query-tags" {
  ...
  group {
    query = "consul`consul`catalog`service`query-tag`*"
    type = "average"
  }
...
resource "circonus_stream_group" "raft-replication-append-entries-logs" {
  ...
  group {
    query = "consul`raft`replication`appendEntries`logs`*"
    type = "average"
  }
...
resource "circonus_stream_group" "raft-replication-append-entries-rpc" {
  ...
  group {
    query = "consul`raft`replication`appendEntries`rpc`*"
    type = "average"
  }
...
resource "circonus_stream_group" "raft-replication-heartbeats" {
  ...
  group {
    query = "consul`raft`replication`heartbeat`*"
    type = "average"
  }
