resource "circonus_graph" "runtime-gc-pause" {
  ...
  stream {
    ...
    stream_name = "${circonus_metric.runtime-gc-pause.name}"
...
resource "circonus_graph" "runtime-gc-pause-line" {
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-total-gc-pause.name}"
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-total-gc-runs.name}"
...
EOF
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-alloc-bytes.name}"
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-sys-bytes.name}"
...
EOF
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-free-count.name}"
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-malloc-count.name}"
...
EOF
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-num-goroutines.name}"
  ...
  stream {
    check = "${circonus_check.consul.check_by_collector[var.collector_id]}"
    stream_name = "${circonus_metric.runtime-heap-objects.name}"
